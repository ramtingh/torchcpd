...
class RigidRegistration(EMRegistration):
	...
	def __init__(self, R=None, t=None, s=None, scale=True, *args, **kwargs):
		...
		self.R = torch.eye(self.D, dtype=self.dtype, device=self.device) if R is None else R
		self.t = torch.from_numpy(np.atleast_2d(np.zeros((1, self.D))) if t is None else t).type(self.dtype).to(self.device)
	...
	def update_transform(self):
		...
		muX = self.PX.sum(axis=0) / self.Np
		# source point cloud mean
		muY = (self.P.T @ self.Y).sum(axis=0) / self.Np

		self.X_hat = self.X - torch.tile(muX, (self.N, 1))
		# centered source point cloud
		Y_hat = self.Y - torch.tile(muY, (self.M, 1))

		self.YPY = (self.P1.T @ torch.multiply(Y_hat, Y_hat).sum(axis=1))
		self.A = (self.X_hat.T @ self.P.T)
		self.A = (self.A @ Y_hat)
		...
		C = torch.ones((self.D,), dtype=self.dtype, device=self.device)
		C[self.D - 1] = torch.linalg.det(U @ V)

		# Calculate the rotation matrix using Eq. 9 of https://arxiv.org/pdf/0905.2635.pdf.
		self.R = ((U @ torch.diag(C)) @ V).T
		# Update scale and translation using Fig. 2 of https://arxiv.org/pdf/0905.2635.pdf.
		if self.scale is True:
			self.s = torch.trace((self.A.T @ self.R.T)) / self.YPY
		else:
			pass
		self.t = muX.T - self.s * (self.R.T @ muY.T)
	...
	def transform_point_cloud(self, Y=None):
		...
		"""
		if Y is None:
			self.TY = self.s * (self.Y @ self.R) + self.t
			return
		else:
			return self.s * (Y @ self.R) + self.t
